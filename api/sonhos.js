...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const filter = email === 'all'
      ? `/sonhos?order=created_at.desc&select=*`
      : `/sonhos?email=eq.${encodeURIComponent(email)}&order=created_at.desc&select=*`;
    const result = await supabaseFetch(filter);
    if (!result.ok) return res.status(500).json({ error: 'Erro ao buscar sonhos.' });
    return res.status(200).json({ success: true, sonhos: result.data || [] });
  }
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'salvar') {
      ...
      const insert = await supabaseFetch('/sonhos', {
        method: 'POST',
        body: JSON.stringify({ email, dados })
      });
      if (!insert.ok) return res.status(500).json({ error: 'Erro ao salvar sonho.' });
    ...
    if (action === 'apagar_um') {
      ...
      const del = await supabaseFetch(
        `/sonhos?id=eq.${id}&email=eq.${encodeURIComponent(email)}`,
        { method: 'DELETE' }
      );
    ...
    if (action === 'apagar') {
      if (!email) return res.status(400).json({ error: 'Email obrigatório.' });
      const del = await supabaseFetch(
        `/sonhos?email=eq.${encodeURIComponent(email)}`,
        { method: 'DELETE' }
      );
      return res.status(del.ok ? 200 : 500).json(del.ok ? { success: true } : { error: 'Erro ao apagar.' });
    }
