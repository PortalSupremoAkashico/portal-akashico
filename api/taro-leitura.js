...
export default async function handler(req, res) {
  ...
  const prompt = `Você é um tarólogo akáshico experiente. Faça uma leitura completa e rica para ${firstName}.

NAIPES DO TARÔ AKÁSHICO:
- CHAMAS = Fogo (Paus): ação, criatividade, vontade, projetos, paixão
- CÁLICES = Água (Copas): emoções, amor, relacionamentos, intuição
- CRISTAIS = Ar (Espadas): mente, conflitos, decisões, verdades
- ESTRELAS = Terra (Ouros): trabalho, dinheiro, manifestação, corpo
- ARCANOS MAIORES: forças universais e lições kármicas da alma
...
RESPONDA EXATAMENTE NESTE FORMATO (sem JSON, sem markdown):

TITULO: [título poético da leitura]

${cartas.map((c, i) => `CARTA_${i+1}
POSICAO: ${c.posicao}
NOME: ${c.nome}${c.invertida ? ' (INVERTIDA)' : ''}
INTERPRETACAO: [escreva 3 parágrafos separados por linha em branco: 1) o que esta carta significa e seus símbolos, 2) como se aplica à pergunta "${perguntaReal}" de ${firstName} nesta posição, 3) conselho dos Registros Akáshicos]`).join('\n\n')}

SINTESE: [3 parágrafos sobre o padrão geral das cartas, a verdade central para ${firstName} e o caminho indicado]

ACAO: [ação concreta e específica para ${firstName} nos próximos dias]`;
