...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    // email=all retorna todas as mentorias (admin)
    let path;
    if (email === 'all') {
      path = `/mentorias?order=created_at.asc&select=id,email,role,content,consulta_tema,consulta_pergunta,created_at`;
    } else {
      path = `/mentorias?email=eq.${encodeURIComponent(email)}&order=created_at.asc&select=id,role,content,consulta_tema,consulta_pergunta,created_at`;
