...
export default async function handler(req, res) {
    ...
    if (action === 'salvar') {
      ...
      const insert = await supabaseFetch('/consultas', {
        ...
        body: JSON.stringify({
          ...
          evolution: (sections?.evolution || '').slice(0, 3000),
  ...
  if (req.method === 'GET') {
    ...
    let path = '/consultas?order=created_at.desc&select=id,nome,email,tema,estado,pergunta,created_at,revelation,earth_future,evolution,technology_future,warning,action';
