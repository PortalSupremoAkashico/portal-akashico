...
// bcrypt cost factor 12 — OWASP recomenda 10-12 (>300ms por hash)
...
function isBcryptHash(h) {
  return typeof h === 'string' && (h.startsWith('$2b$') || h.startsWith('$2a$'));
}
...
export default async function handler(req, res) {
  ...
  if (action === 'cadastro') {
    ...
    const novoHash = await bcrypt.hash(senhaInput, BCRYPT_ROUNDS);
  ...
  if (action === 'login') {
    ...
    if (isBcryptHash(stored)) {
      // Hash bcrypt — verifica normalmente
      if (senhaInput) {
        try { ok = await bcrypt.compare(senhaInput, stored); } catch { ok = false; }
      }
    } else if (isSha256Hash(stored)) {
      ...
      // Se autenticou com hash legado, migra para bcrypt
      if (ok && senhaInput) {
        const novoHash = await bcrypt.hash(senhaInput, BCRYPT_ROUNDS);
