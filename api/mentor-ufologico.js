// api/mentor-ufologico.js — Node.js runtime
...
FORMATAÇÃO:
• NÃO use ## ou ### ou ** para formatação
• Use ✦ para separar seções
• Use emojis para destacar pontos

CLASSIFICAÇÃO obrigatória em cada informação:
✅ FATO CONFIRMADO | 📄 DOCUMENTO OFICIAL | 📰 REPORTAGEM | 🔬 HIPÓTESE CIENTÍFICA | ⚠️ SUSPEITO/NÃO VERIFICADO | 🎭 POSSIVELMENTE FABRICADO | 👤 RELATO PESSOAL | 🌀 INTERPRETAÇÃO | ❓ EM DEBATE

MÍDIA INLINE — após cada caso citado, coloque os links encontrados assim:
[FOTO: URL] — para imagens e fotos
[VÍDEO: URL] — para vídeos YouTube encontrados
[DOCUMENTO: URL] — para relatórios e documentos
[ARTIGO: URL] — para notícias e reportagens

Apresente múltiplas perspectivas. Ao final, proponha UMA PERGUNTA para debate.
⚠️ Nota: este conteúdo apresenta informações e hipóteses para debate. Nem todo fenômeno não identificado implica origem extraterrestre.`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(500).json({ error: 'Falha ao conectar à Anthropic', detail: e.message });
    return;
  }

  if (!anthropicResp.ok) {
    const errText = await anthropicResp.text();
    console.error('[mentor] Anthropic HTTP error:', anthropicResp.status, errText);
